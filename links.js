...
const SECTIONS = [
  {
    ...
    items: [
      { label: "Entity (REST)",    icon: "cube",   path: (c) => c.entityId ? `/api/entities/${c.entityId}` : null },
  ...
  },
  {
    ...
    defaultCollapsed: false,
    items: [
      { label: "Definition by name (REST)",    icon: "schema", path: (c) => c.definitionName ? `/api/entitydefinitions/${c.definitionName}` : null },
      { label: "Definition by name v2 (REST)", icon: "schema", path: (c) => c.definitionName ? `/api/entitydefinitions/name/${c.definitionName}` : null },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "All entity definitions", icon: "schema",     path: "/api/entitydefinitions" },
